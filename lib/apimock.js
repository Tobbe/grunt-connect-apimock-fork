...
function advancedMockformat(req, res, mockdata){
    ...
    req.on('end', function(){
        ...
        if(parseError){
            ...
            response = selectResponse(res, mockdata, requestParameters, bodyParameters, req.headers);
        }
        respondData(res, response);
    });
}
...
function selectResponse(res, mockdata, requestParameters, bodyParameters, headers) {
    ...
    for (var i = 0; i < mockresponses.length; i++) {
        ...
        if (parametersMatch && bodyMatch && headersMatch) {
